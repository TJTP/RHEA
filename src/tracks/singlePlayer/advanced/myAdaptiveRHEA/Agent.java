...
public class Agent extends AbstractPlayer {
    ...
    public Agent(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        this.heuristic = new MyHeuristic(this.recognizeGameType(stateObs));
        this.randomGenerator = new Random();
        this.tuner = new Tuner(SIMULATION_DEPTH);
    ...
    public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        while (this.globalRemaining > this.avgTimeTaken && this.globalRemaining > BREAK_MS && this.keepIterating) {
            this.tuner.assignParams();
            double fitnessDiff = runIteration(stateObs);
            this.tuner.updateProb(fitnessDiff);
            this.globalRemaining = this.timer.remainingTimeMillis();
        }
     ...
     */
    private double runIteration(StateObservation stateObs) {
        ...
        if (this.indNum > 1) {
            ...
            for (int i = ELITISM; i < POPULATION_SIZE; i++) {
                if (this.globalRemaining > 2 * this.avgTimeTakenEval && this.globalRemaining > BREAK_MS) {
                    ...
                    Individual newInd;
                    if (this.tuner.doCrossover) {
                        newInd = crossover();
                    } else {
                        newInd = this.population[i % this.indNum].copy();
                    }

                    if (this.tuner.doMutation) {
                        newInd.mutate(this.tuner.mutationNum);
                    }
        ...
        double fitnessDiff = this.nextPop[0].value - this.population[0].value;
    ...
    private Individual crossover() {
        ...
        if (this.indNum > 1) {
            ...
            if (TOURNAMENT_SIZE >= 2) {
                newInd.crossover(tournament[0], tournament[1], this.tuner.crossoverType);
            } else {
